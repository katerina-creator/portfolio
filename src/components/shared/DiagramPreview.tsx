...
export default function DiagramPreview({ src, alt = '', caption }: Props) {
  ...
  return (
    <>
      <figure className="diagram-preview__wrapper">
        <div
          className="diagram-preview__card"
          onClick={() => setOpen(true)}
          role="button"
          tabIndex={0}
          aria-label={`View ${alt || 'diagram'} fullscreen`}
          onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') setOpen(true) }}
        >
          <img src={src} alt={alt} />
          <div className="diagram-preview__zoom-hint" aria-hidden="true">
        ...
        {caption && <figcaption className="diagram-preview__caption">{caption}</figcaption>}
      </figure>
